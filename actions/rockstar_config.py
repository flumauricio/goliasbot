"""
View de configuração do canal Rockstar para o !setup.
Arquivo: actions/rockstar_config.py
"""
...
class RockstarSetupView(discord.ui.View):
    """View para configurar o canal do sistema de contas Rockstar."""
    ...
    def __init__(self, bot: commands.Bot, db: Database, guild: discord.Guild, parent_view=None):
        ...
        self.channel_select = discord.ui.ChannelSelect(
            placeholder="Selecione o canal de contas Rockstar...",
    ...
    async def build_embed(self) -> discord.Embed:
        ...
        embed = await build_standard_config_embed(
            title="🎮 Configuração do Sistema de Contas Rockstar",
            description=(
                "Configure o canal onde os usuários poderão cadastrar e gerenciar suas contas Rockstar.\n\n"
                "**Como funciona:**\n"
                "• Digite `email:senha:chave2fa` no canal para cadastrar\n"
                "• Múltiplas contas por linha são aceitas\n"
                "• A mensagem é apagada imediatamente por segurança\n"
                "• Use `!contas` no canal para gerenciar suas contas"
            ),
            current_config={"Canal Rockstar": channel_text},
            guild=self.guild,
    ...
    async def on_channel_select(self, interaction: discord.Interaction):
        ...
        if not channel:
            ...
            return
        await self.db.upsert_settings(self.guild.id, channel_rockstar=channel.id)
        embed = await self.build_embed()
        try:
            await interaction.message.edit(embed=embed, view=self)
        except discord.NotFound:
            pass
        await interaction.followup.send(f"✅ Canal Rockstar configurado: {channel.mention}", ephemeral=True)
    ...
    async def remove_channel(self, interaction: discord.Interaction):
        ...
        try:
            async with self.db._conn.cursor() as cur:
                await cur.execute(
                    "UPDATE settings SET channel_rockstar = NULL WHERE guild_id = ?",
                    (self.guild.id,),
                )
                await self.db._conn.commit()
        except Exception as e:
            LOGGER.error("Erro ao remover canal Rockstar: %s", e)
        ...
        try:
            ...
            pass
        await interaction.followup.send("✅ Canal Rockstar removido.", ephemeral=True)
